...
export function useLoadInitialSearchParams(
  ...
  useEffect(() => {
    ...
    const loadUrlParams = async () => {
      ...
      let locationType = searchParams.get("location_type");
      const lat = searchParams.get("lat");
      const lng = searchParams.get("lng");
      const radius = searchParams.get("r");
      const facets = searchParams.get("facets");

      //If there are no initial URL params, use country as default search on load
      if (!lat && !lng && !query && !locationType && !facets) {
